...
def add_general_google_play_arguments(parser):
    parser.add_argument("--credentials", dest="google_play_credentials_filename", help="The json authentication file", required=True)

    parser.add_argument("--commit", action="store_true", help="Commit changes onto Google Play. This action cannot be reverted.")
    parser.add_argument(
        "--do-not-contact-google-play",
        action="store_false",
        dest="contact_google_play",
...
class GooglePlayEdit:
    ...
    def get_track_status(self, track):
        response = self._edit_resource.tracks().get(editId=self._edit_id, track=track, packageName=self._package_name).execute(num_retries=NUM_RETRIES)
    ...
    def upload_apk(self, apk):
        ...
        try:
            ...
            if e.resp["status"] == "403":
                ...
                if len(errors) == 1 and errors[0]["reason"] in ("apkUpgradeVersionConflict", "apkNotificationMessageKeyUpgradeVersionConflict"):
                    logger.warning('APK "{}" has already been uploaded on Google Play. Skipping...'.format(apk_path))
                    return
            raise
    ...
    def _update_track(self, track, version_codes, rollout_percentage=None):
        if track == "rollout" and rollout_percentage is None:
            raise WrongArgumentGiven("To perform a rollout, you must provide the target track (probably 'production') and a rollout_percentage")
        if rollout_percentage is not None:
            ...
            release = {
                "status": "inProgress",
                "userFraction": rollout_percentage / 100.0,  # Ensure float in Python 2
                "versionCodes": version_codes,
            }
        else:
            release = {"status": "completed", "versionCodes": version_codes}
        ...
        response = (
            self._edit_resource.tracks().update(editId=self._edit_id, track=track, packageName=self._package_name, body=body).execute(num_retries=NUM_RETRIES)
        )
    ...
    def update_listings(self, language, title, full_description, short_description):
        ...
        response = (
            self._edit_resource.listings()
            .update(editId=self._edit_id, packageName=self._package_name, language=language, body=body)
            .execute(num_retries=NUM_RETRIES)
        )
        logger.info('Listing for language "{}" has been updated with: {}'.format(language, body))
        logger.debug("Listing response: {}".format(response))

    def update_whats_new(self, language, apk_version_code, whats_new):
        response = (
            self._edit_resource.apklistings()
            .update(editId=self._edit_id, packageName=self._package_name, language=language, apkVersionCode=apk_version_code, body={"recentChanges": whats_new})
            .execute(num_retries=NUM_RETRIES)
        )
        logger.info('What\'s new listing for ("{}", "{}") has been updated to: "{}"'.format(language, apk_version_code, whats_new))
        logger.debug("Apk listing response: {}".format(response))
...
def _create_google_edit_resource(contact_google_play, credentials_file_name):
    if contact_google_play:
        ...
        service = build(serviceName="androidpublisher", version="v3", credentials=credentials, cache_discovery=False, num_retries=NUM_RETRIES)
        ...
        edit_resource_mock.insert = lambda *args, **kwargs: _ExecuteDummy({"id": "fake-transaction-id"})
        ...
        bundles_mock.upload = lambda *args, **kwargs: _ExecuteDummy({"versionCode": "fake-version-code"})

...
def get_expected_combos(firefox_version, package_name):
    ...
    if not combos:
        raise ValueError(
            "No combos found for Firefox version {}. Current rules: {}".format(firefox_version, _MAJOR_FIREFOX_VERSIONS_PER_ARCHITECTURE_AND_API_LEVEL)
        )

    logger.debug("Expected to find these combos for Firefox {}: {}".format(firefox_version, craft_combos_pretty_names(combos)))
    return combos
...
def get_expected_api_levels(firefox_version, architecture="armeabi-v7a", package_name="org.mozilla.firefox"):
    return [
        ...
        if (
            _is_firefox_version_in_range(firefox_version, range_dict)
            or
            # XXX arm64-v8a shipped on Nightly between 66 and 67, and on Beta on 67.
            (
                architecture == "arm64-v8a"
                and (get_firefox_major_version_number(firefox_version) in _EXCEPTIONALLY_SHIPPED_MAJOR_NUMBERS_PER_PACKAGE_NAME.get(package_name, ()))
            )
        )
    ]
...
def craft_combos_pretty_names(combos):
    return ", ".join(["{} API {}+".format(*combo) for combo in combos])

...
def cross_check_apks(
    apks_metadata, expected_package_names, skip_checks_fennec, skip_check_multiple_locales, skip_check_same_locales, skip_check_ordered_version_codes
):
    ...
    if not skip_checks_fennec:
        ...
        _check_version_matches_package_name(singular_apk_metadata["firefox_version"], singular_apk_metadata["package_name"])
...
def _check_package_names(expected_package_names, apks_metadata):
    ...
    if not types == set(expected_package_names):
        raise BadSetOfApks("Expected package names {}, found {}".format(expected_package_names, types))
...
def _check_version_matches_package_name(version, package_name):
    ...
    if (
        (package_name == "org.mozilla.firefox" and sanitized_version.is_release)
        or
        # Due to project Dawn, Nightly is now using the Aurora package name. See bug 1357351.
        (package_name == "org.mozilla.fennec_aurora" and sanitized_version.is_nightly)
        or (
...
def _check_apks_version_codes_are_correctly_ordered(apks_metadata):
    architectures_per_version_code = {metadata["version_code"]: metadata["architecture"] for metadata in apks_metadata.values()}

    if len(architectures_per_version_code) != len(apks_metadata):
        raise BadSetOfApks("Some APKs are sharing the same version code! APKs metadata: {}".format(apks_metadata))
...
def _check_all_architectures_and_api_levels_are_present(apks_metadata):
    ...
    current_combos = set([(metadata["architecture"], metadata["api_level"]) for metadata in apks_metadata.values()])

    missing_combos = expected_combos - current_combos
    if missing_combos:
        raise BadSetOfApks("One or several APKs are missing for Firefox {}: {}".format(firefox_version, craft_combos_pretty_names(missing_combos)))

    extra_combos = current_combos - expected_combos
    if extra_combos:
        raise BadSetOfApks(
            "One or several APKs are not allowed for Firefox {}: {}. \
Please make sure mozapkpublisher has allowed them to be uploaded.".format(firefox_version, craft_combos_pretty_names(extra_combos))
        )

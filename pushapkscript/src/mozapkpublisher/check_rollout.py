...
from mozapkpublisher.common.store import GooglePlayEdit, add_general_google_play_arguments
...
def main():
    ...
    parser.add_argument("--days", help="The time before we warn about incomplete staged rollout of a release (default: 7)", type=int, default=7)
    config = parser.parse_args()

    with GooglePlayEdit.transaction(config.google_play_credentials_filename, "org.mozilla.firefox", contact_server=True, dry_run=True) as edit:
        for release, age in check_rollout(edit, config.days):
            print(
                "fennec {} is on staged rollout at {}% but it shipped {} days ago".format(release["name"], int(release["userFraction"] * 100), int(age / DAY))
            )

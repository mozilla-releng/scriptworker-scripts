...
def verify(product_config, apk_path):
    ...
    if not _does_apk_have_expected_digest(apk_path, expected_digest_algorithm):
        raise SignatureError('Wrong digest algorithm: "{}" digest is expected, but it was not found'.format(expected_digest_algorithm))

    log.info('The signature of "{}" contains the correct digest algorithm ({})'.format(apk_path, expected_digest_algorithm))

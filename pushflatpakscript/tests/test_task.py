...
def test_get_flatpak_channel_dep(raises, channel):
    ...
    config = {"taskcluster_scope_prefix": "project:releng:flathub:firefox:", "push_to_flathub": False}
    if raises:
        with pytest.raises(TaskVerificationError):
            get_flatpak_channel(config, task)
    else:
...
def test_get_flatpak_channel_prod(raises, scopes, channel):
    ...
    config = {"taskcluster_scope_prefix": "project:releng:flathub:firefox:", "push_to_flathub": True}
    if raises:
        with pytest.raises(TaskVerificationError):
            get_flatpak_channel(config, task)
    else:
...
def test_is_allowed_to_push_to_flathub(channel, push_to_flathub, expected):
    config = {"taskcluster_scope_prefix": "project:releng:flathub:firefox:", "push_to_flathub": push_to_flathub}

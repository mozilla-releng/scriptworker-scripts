...
def validate_mar_channel(context, mar):
    ...
    if any(fnmatch.fnmatch(channel_id, pattern) for pattern in context.mar_channels.get(cert_type, [])):
        return
    raise SigningScriptError(f"Cannot use mar channel id {channel_id}")

...
@pytest.mark.asyncio
async def test_async_main_git_push(git_push_task, mocker):
    mock_build_decision = mocker.patch("builddecisionscript.git_push.build_decision")

    await async_main({}, git_push_task)

    mock_build_decision.assert_called_once()
    call_kwargs = mock_build_decision.call_args.kwargs
    assert call_kwargs["hook_payload"] == git_push_task["payload"]["hookPayload"]
    assert call_kwargs["dry_run"] is False


@pytest.mark.asyncio
async def test_async_main_git_push_dry_run(git_push_task, mocker):
    git_push_task["payload"]["dryRun"] = True
    mock_build_decision = mocker.patch("builddecisionscript.git_push.build_decision")
...
@pytest.mark.asyncio
async def test_async_main_git_push_missing_hook_payload(git_push_task, mocker):
    del git_push_task["payload"]["hookPayload"]
    mocker.patch("builddecisionscript.git_push.build_decision")

    with pytest.raises(ValueError, match="hookPayload is required"):
        await async_main({}, git_push_task)

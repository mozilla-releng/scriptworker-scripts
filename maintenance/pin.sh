...
if [ ${#PY38_DIRS} -gt 0 ]; then
    ...
    echo "${PY38_DIRS[@]}" | xargs -n4 -P4 time docker run --platform linux/x86_64 --rm -t -v "$PWD":/src -e EXTRA_ARGS="$EXTRA_ARGS" -e SUFFIX=py38.txt -w /src scriptworker-script-pin:3.8 maintenance/pin-helper.sh
fi
if [ ${#PY311_DIRS} -gt 0 ]; then
    ...
    echo "${PY311_DIRS[@]}" | xargs -n4 -P4 time docker run --platform linux/x86_64 --rm -t -v "$PWD":/src -e EXTRA_ARGS="$EXTRA_ARGS" -w /src scriptworker-script-pin:3.11 maintenance/pin-helper.sh
fi

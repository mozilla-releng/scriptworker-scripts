...
def test_check_and_extract_zip_archives():
    first_task_id_archive1_files_and_content = {"some_file1": "some content 1", "some/subfolder/file1": "some other content 1"}
    first_task_id_archive2_files_and_content = {"some_file2": "some content 2", "some/subfolder/file2": "some other content 2"}
    third_task_id_archive1_files_and_content = {"some_file3": "some content 3", "some/subfolder/file3": "some other content 3"}

    with tempfile.TemporaryDirectory() as d:
        first_task_id_archive1_path = _create_zip(d, first_task_id_archive1_files_and_content, archive_name="firstTaskId-archive1.zip")
        first_task_id_archive2_path = _create_zip(d, first_task_id_archive2_files_and_content, archive_name="firstTaskId-archive2.zip")
        third_task_id_archive1_path = _create_zip(d, third_task_id_archive1_files_and_content, archive_name="thirdTaskId-archive1.zip")

        artifacts_per_task_id = {
            "firstTaskId": [
                {"paths": ["/a/non/archive", "/another/non/archive"], "zip_extract": False},
                {"paths": [first_task_id_archive1_path, first_task_id_archive2_path], "zip_extract": True},
            ],
            "secondTaskId": [{"paths": ["/just/another/regular/file"], "zip_extract": False}],
            "thirdTaskId": [{"paths": [third_task_id_archive1_path], "zip_extract": True}],
        }
        ...
        expected_files_per_archive_per_task_id = {
            ...
            "thirdTaskId": {third_task_id_archive1_path: list(third_task_id_archive1_files_and_content.keys())},
        }

        mapping_manifest = {"s3_bucket_path": "dummy"}
        files_once_extracted = check_and_extract_zip_archives(
            artifacts_per_task_id, expected_files_per_archive_per_task_id, zip_max_size_in_mb=100, mapping_manifest=mapping_manifest
        )
...
def test_check_and_extract_zip_archives_for_given_task():
    archive1_files_and_content = {"some_file1": "some content 1", "some/subfolder/file1": "some other content 1"}
    archive2_files_and_content = {"some_file2": "some content 2", "some/subfolder/file2": "some other content 2"}
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        expected_files_per_archive = {archive1_path: ["some_file1", "some/subfolder/file1"], archive2_path: ["some_file2", "some/subfolder/file2"]}

        mapping_manifest = {"s3_bucket_path": "dummy"}
        extracted_files = _check_and_extract_zip_archives_for_given_task(
            "someTaskId", expected_files_per_archive, zip_max_size_in_mb=100, mapping_manifest=mapping_manifest
        )
...
def test_check_extract_and_delete_zip_archive():
    files_and_content = {"some_file": "some content", "some/subfolder/file": "some other content"}

    with tempfile.TemporaryDirectory() as d:
        archive_path = _create_zip(d, files_and_content)
        mapping_manifest = {"s3_bucket_path": "dummy"}
        extracted_files = _check_extract_and_delete_zip_archive(
            archive_path, files_and_content.keys(), zip_max_size_in_mb=100, mapping_manifest=mapping_manifest
        )
        assert extracted_files == {
...
@pytest.mark.parametrize(
    "file_size_in_mb, is_zip, zip_max_size_in_mb, raises", ((1, True, 2, False), (3, True, 2, True), (3, True, 4, False), (1, False, 2, True))
)
    ...
    with tempfile.NamedTemporaryFile(mode="w+b") as f:
        ...
        with zipfile.ZipFile(f.name, mode="r") as zip_file:
            assert _fetch_zip_metadata(zip_file) == {
                "some_file": {"compress_size": 67, "file_size": 32},
                "some/subdir/some_other_file": {"compress_size": 72, "file_size": 38},
            }


@pytest.mark.parametrize(
    "zip_metadata, zip_max_size_in_mb, raises",
    (
        ({"file1": {"compress_size": 1000, "file_size": 1000}, "file2": {"compress_size": 1000, "file_size": 2000}}, 100, False),
        ({"file1": {"compress_size": 101 * 1024 * 1024, "file_size": 101 * 1024 * 1024}}, 100, True),
        ({"file1": {"compress_size": 2 * 1024 * 1024, "file_size": 2 * 1024 * 1024}}, 1, True),
        ({"file1": {"compress_size": 100, "file_size": 1 * 1024 * 1024}}, 100, True),
        ({"file1": {"compress_size": 50, "file_size": 10}}, 100, False),  # Can happen with small files
    ),
)
...
@pytest.mark.parametrize(
    "files_in_archive, expected_files, mapping_manifest, raises",
    (
        (["some_file", "some/other/file"], ["some_file", "some/other/file"], {"s3_bucket_path": "dummy"}, False),
        (["/some/absolute/path"], ["/some/absolute/file"], {"s3_bucket_path": "dummy"}, True),
        (["some/.///redundant/path"], ["some/.///redundant/path"], {"s3_bucket_path": "dummy"}, True),
        (["some/../../../etc/passwd"], ["some/../../../etc/passwd"], {"s3_bucket_path": "dummy"}, True),
        (["some_file", "some_wrong_file"], ["some_file", "some_other_file"], {"s3_bucket_path": "dummy"}, True),
        (["some_file"], ["some_file", "some_missing_file"], {"s3_bucket_path": "dummy"}, True),
        (["some_file", "some_unexpected_file"], ["some_file"], {"s3_bucket_path": "dummy"}, True),
        (["some_duplicated_file", "some_duplicated_file"], ["some_duplicated_file", "some_duplicated_file"], {"s3_bucket_path": "dummy"}, True),
        (
            [
            ...
            [
                ...
                "org/mzl/components/browser-awesomebar/X.Y.Z-SNAPSHOT/browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}.aar.sha1",  # noqa: E501
            ],
        ...
        (
            ...
            [
                "org/mzl/components/browser-awesomebar/X.Y.Z-SNAPSHOT/browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}-sources.jar",  # noqa: E501
                "org/mzl/components/browser-awesomebar/X.Y.Z-SNAPSHOT/browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}-sources.jar.md5",  # noqa: E501
            ],
            {
                "s3_bucket_path": "dummy-SNAPSHOT",
                "mapping": {
                    "en-US": {
                        "browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}-sources.jar": {
                            "destinations": ["browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}-sources.jar"],
                        ...
                        "browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}-sources.jar.md5": {
                            "destinations": ["browser-awesomebar-X.Y.Z-{{date_timestamp}}.{{clock_timestamp}}-{{build_number}}-sources.jar.md5"],
...
def test_ensure_all_expected_files_are_present_in_archive(files_in_archive, expected_files, mapping_manifest, raises):
    if raises:
        with pytest.raises(TaskVerificationError):
            _ensure_all_expected_files_are_present_in_archive("/some/archive.zip", files_in_archive, expected_files, mapping_manifest)
    else:
        _ensure_all_expected_files_are_present_in_archive("/some/archive.zip", files_in_archive, expected_files, mapping_manifest)
...
def test_extract_and_check_output_files():
    with tempfile.TemporaryDirectory() as d:
        ...
        expected_extracted_files = {"some_file": extracted_file1, "some/subfolder/file": extracted_file2}

        with zipfile.ZipFile(zip_path, mode="r") as zip_file:
            assert _extract_and_check_output_files(zip_file, ["some_file", "some/subfolder/file"]) == expected_extracted_files
...
@pytest.mark.parametrize(
    "files, raises", ((["/some/file"], False), (["/some/file", "/some/other_file"], False), (["/some/file", "/some/other_file", "/some/file"], True))
)

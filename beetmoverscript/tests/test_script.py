...
async def test_push_to_maven(context, mocker, extract_zip_output, ErrorRaised):
    ...
    mocker.patch("beetmoverscript.maven_utils.get_maven_expected_files_per_archive_per_task_id", new=lambda _, **kwargs: ("", {}))
    mocker.patch("beetmoverscript.zip.check_and_extract_zip_archives", new=lambda _, __, ___, ____: extract_zip_output)
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "artifact_map, extract_zip_output, ErrorRaised",
    (
        (
            [
                {

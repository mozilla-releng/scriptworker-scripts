...
@pytest.mark.parametrize(
    ...
    (
        pytest.param(
            ...
            None,
            id="one_file_one_dest_default_overwrites",
        ),
        pytest.param(
            [
                {
...
async def test_upload_translations_artifacts(aioresponses, monkeypatch, context, upstream_artifacts, artifact_map, expected_uploads, allow_overwrites):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if allow_overwrites:
                # When allow_overwrites is True, if_generation_match should not be set or be None
                for call in blob.upload_from_filename.call_args_list:
                    assert call[1].get("if_generation_match") is None
            elif allow_overwrites is False:
                # When allow_overwrites is False (explicit or default), if_generation_match should be 0
                for call in blob.upload_from_filename.call_args_list:

...
@pytest.mark.parametrize(
    "upstream_artifacts_per_task_id, mapping_manifest, expected, raises",
    (
        (
            {"someTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip"]}]},
            {
                "mapping": {
                    "en-US": {
        ...
        (
            {"someTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.jsshell.zip"]}]},
            {
                "mapping": {
                    "en-US": {
                        "geckoview-beta-x86-62.0b3.aar": {},
        ...
        (
            {
                "someTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip"]}],
                "someOtherTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip"]}],
            },
            {
                "mapping": {
                    "en-US": {
        ...
        (
            {
                "someTaskId": [
                    {"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip", "/work_dir/cot/someTaskId/public/build/other/folder/target.maven.zip"]}
                ]
            },
            {
                "mapping": {
...
def test_get_maven_expected_files_per_archive_per_task_id(upstream_artifacts_per_task_id, mapping_manifest, expected, raises):
    if raises:
        with pytest.raises(ValueError):
            get_maven_expected_files_per_archive_per_task_id(upstream_artifacts_per_task_id, mapping_manifest=mapping_manifest)
    else:
        assert get_maven_expected_files_per_archive_per_task_id(upstream_artifacts_per_task_id, mapping_manifest=mapping_manifest) == expected


@pytest.mark.parametrize(
    "upstream_artifacts_per_task_id, artifact_map, expected, raises",
    (
        (
            {"someTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip"]}]},
            [
        ...
        ({"someTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip"]}]}, [{"taskId": "someTaskId", "no_paths": {}}], {}, KeyError),
        (
            {"someTaskId": [{"paths": ["/work_dir/cot/someTaskId/public/build/target.maven.zip"]}]},
            [{"taskId": "someTaskId", "paths": "not a dict"}],
            {},
            AttributeError,
        ),
    ),
...
def test_get_maven_expected_files_per_archive_per_task_id_with_map(upstream_artifacts_per_task_id, artifact_map, expected, raises):
    if raises:
        with pytest.raises(raises):
            get_maven_expected_files_per_archive_per_task_id(upstream_artifacts_per_task_id, artifact_map=artifact_map)
    else:
        assert get_maven_expected_files_per_archive_per_task_id(upstream_artifacts_per_task_id, artifact_map=artifact_map) == expected

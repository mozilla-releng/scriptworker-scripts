...
from beetmoverscript.constants import SNAPSHOT_TIMESTAMP_REGEX, ZIP_MAX_COMPRESSION_RATIO
from beetmoverscript.utils import JINJA_ENV

log = logging.getLogger(__name__)


def check_and_extract_zip_archives(artifacts_per_task_id, expected_files_per_archive_per_task_id, zip_max_size_in_mb, mapping_manifest):
    ...
    for task_id, task_artifacts_params in artifacts_per_task_id.items():
        for artifacts_param in task_artifacts_params:
            ...
            deflated_artifacts.update(_check_and_extract_zip_archives_for_given_task(task_id, expected_files_per_archive, zip_max_size_in_mb, mapping_manifest))
...
def _check_and_extract_zip_archives_for_given_task(task_id, expected_files_per_archive, zip_max_size_in_mb, mapping_manifest):
    extracted_files = {}
    ...
    for archive_path, expected_files in expected_files_per_archive.items():
        ...
        extracted_files[archive_path] = _check_extract_and_delete_zip_archive(archive_path, expected_files, zip_max_size_in_mb, mapping_manifest)
...
def _check_extract_and_delete_zip_archive(zip_path, expected_files, zip_max_size_in_mb, mapping_manifest):
    ...
    with zipfile.ZipFile(zip_path) as zip_file:
        ...
        _ensure_all_expected_files_are_present_in_archive(zip_path, relative_paths_in_archive, expected_files, mapping_manifest)
...
def _check_archive_itself(zip_path, zip_max_size_in_mb):
    ...
    if zip_size_in_mb > zip_max_size_in_mb:
        raise TaskVerificationError(
            'Archive "{}" is too big. Max accepted size (in MB): {}. File size (in MB): {}'.format(zip_path, zip_max_size_in_mb, zip_size_in_mb)
        )
...
def _fetch_zip_metadata(zip_file):
    return {
        info.filename: {"compress_size": info.compress_size, "file_size": info.file_size}
...
def _ensure_all_expected_files_are_present_in_archive(zip_path, files_in_archive, expected_files, mapping_manifest):
    ...
    if len(expected_files) != len(unique_expected_files):
        ...
        raise TaskVerificationError('Found duplicated expected files in archive "{}": {}'.format(zip_path, duplicated_files))
    ...
    for file_ in files_in_archive:
        if os.path.isabs(file_):
            raise TaskVerificationError('File "{}" in archive "{}" cannot be an absolute one.'.format(file_, zip_path))
        if os.path.normpath(file_) != file_:
            raise TaskVerificationError('File "{}" in archive "{}" cannot contain up-level reference nor redundant separators'.format(file_, zip_path))
        if mapping_manifest and "SNAPSHOT" in mapping_manifest["s3_bucket_path"]:
            (date, clock, bno) = _extract_and_check_timestamps(file_, SNAPSHOT_TIMESTAMP_REGEX)
            _args = {"date_timestamp": date, "clock_timestamp": clock, "build_number": bno}
            # reload the unique_expected_files with their corresponding values
            # by rendering them via Jinja2 variables
            rendered_unique_expected_files = set([JINJA_ENV.from_string(f).render(**_args) for f in unique_expected_files])
            identifiers_collection.append(frozenset(_args.items()))
        if file_ not in rendered_unique_expected_files:
            raise TaskVerificationError('File "{}" present in archive "{}" is not expected. Expected: {}'.format(file_, zip_path, unique_expected_files))
    ...
    if len(identifiers_collection_set) > 1:
        ...
        raise TaskVerificationError("Different buildnumbers/timestamps identified within the archive {}".format(zip_path))
    ...
    if len(files_in_archive) != len(unique_expected_files):
        ...
        raise TaskVerificationError('Expected files are missing in archive "{}": {}'.format(zip_path, missing_expected_files))
...
def _extract_and_check_timestamps(archive_filename, regex):
    ...
    try:
        ...
        raise TaskVerificationError(
            'File "{}" present in archive has invalid identifier. ' "Expected YYYYMMDD.HHMMSS-BUILDNUMBER within in".format(archive_filename)
        )
    timestamp, build_number = identifier.split("-")
    try:
        datetime.datetime.strptime(timestamp, "%Y%m%d.%H%M%S")
    except ValueError:
        raise TaskVerificationError('File "{}" present in archive has invalid timestamp. ' "Expected YYYYMMDD.HHMMSS within in".format(archive_filename))
...
def _extract_and_check_output_files(zip_file, relative_path_in_archive):
    ...
    if not os.path.isabs(zip_path):
        raise TaskVerificationError('Archive "{}" is not absolute path. Cannot know where to extract content'.format(zip_path))

    extract_to = "{}.out".format(zip_path)
    expected_full_paths_per_relative_path = {path_in_archive: os.path.join(extract_to, path_in_archive) for path_in_archive in relative_path_in_archive}
...
def _ensure_all_expected_files_are_deflated_on_disk(zip_path, expected_full_paths):
    for full_path in expected_full_paths:
        if not os.path.exists(full_path):
            raise TaskVerificationError('After extracting "{}", expected file "{}" does not exist'.format(zip_path, full_path))
        if not os.path.isfile(full_path):
            raise TaskVerificationError('After extracting "{}", "{}" is not a file'.format(zip_path, full_path))

    log.info('All files declared in archive "{}" exist and are regular files: {}'.format(zip_path, expected_full_paths))
...
def _ensure_no_file_got_overwritten(task_id, extracted_files):
    ...
    if len(unique_paths) != len(extracted_files):
        ...
        raise TaskVerificationError('Archives from task "{}" overwrote files: {}'.format(task_id, duplicated_paths))

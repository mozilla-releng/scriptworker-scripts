...
def get_maven_expected_files_per_archive_per_task_id(upstream_artifacts_per_task_id, mapping_manifest=None, artifact_map=None):
    ...
    if artifact_map:
        return {task_id: {maven_zip_full_path: _get_maven_expected_files_from_map(artifact_map, task_id)}}
    else:
        return {task_id: {maven_zip_full_path: _get_maven_expected_files_in_archive(mapping_manifest)}}
...
def _get_task_id_and_full_path_of_maven_archive(upstream_artifacts_per_task_id):
    ...
    if not candidate_task_id:
        raise ValueError('No upstream artifact ending with "{}" found. Given: {}'.format(_MAVEN_ZIP_SUFFIX, upstream_artifacts_per_task_id))
...
def _get_maven_expected_files_in_archive(mapping_manifest):
    ...
    return [os.path.join(_remove_first_directory_from_bucket(mapping_manifest["s3_bucket_path"]), file) for file in files]

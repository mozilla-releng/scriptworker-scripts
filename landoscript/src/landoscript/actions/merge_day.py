...
async def run(github_client: GithubClient, public_artifact_dir, merge_info):
    ...
    if replacements or regex_replacements:
        ...
        for fn in orig_contents:
            diff += diff_contents(orig_contents[fn], new_contents[fn], fn)
...
async def get_version(github_client: GithubClient, version_file, branch):

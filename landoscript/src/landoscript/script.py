...
from landoscript.actions import android_l10n_import, l10n_bump, merge_day, tag, version_bump
...
def validate_scopes(scopes, lando_repo, actions):
    expected_scopes = {
        f"project:releng:landoscript:repo:{lando_repo}",
        *[f"project:releng:landoscript:action:{action}" for action in actions],
    }
            ...
            for action in payload["actions"]:
                if action == "version_bump":
                    ...
                    import_action = await android_l10n_import.run(
                        gh_client, context.config["github_config"], public_artifact_dir, branch, android_l10n_import_info
                    )
                    if import_action:

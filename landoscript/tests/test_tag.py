...
def setup_test(github_installation_responses, context, payload, repo="repo_name"):
    ...
    scopes = [
        f"project:releng:landoscript:repo:{repo}",
        f"project:releng:landoscript:action:tag",
    ]
...
async def test_missing_scopes(context):
    ...
    context.task = {"payload": payload, "scopes": ["project:releng:landoscript:repo:repo_name"]}
    ...
    try:
        ...
        assert "project:releng:landoscript:action:tag" in e.args[0]

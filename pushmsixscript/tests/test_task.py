...
def test_get_msix_channel_dep(raises, channel):
    ...
    config = {"taskcluster_scope_prefix": "project:releng:microsoftstore:", "push_to_store": False}
    if raises:
        with pytest.raises(TaskVerificationError):
            get_msix_channel(config, task)
    else:
...
def test_get_msix_channel_prod(raises, scopes, channel):
    ...
    config = {"taskcluster_scope_prefix": "project:releng:microsoftstore:", "push_to_store": True}
    if raises:
        with pytest.raises(TaskVerificationError):
            get_msix_channel(config, task)
    else:
...
def test_is_allowed_to_push_to_microsoft_store(channel, push_to_store, expected):
    config = {"taskcluster_scope_prefix": "project:releng:microsoftstore:", "push_to_store": push_to_store}

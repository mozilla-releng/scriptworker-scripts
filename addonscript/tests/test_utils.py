...
def payload():
    ...
    with freeze_time("2018-01-19 12:59:59"):
        _iat = int(time.time())
        payload = {"iss": "test-user", "jti": str(uuid.uuid4()), "iat": _iat, "exp": _iat + 60 * 4}
    return payload
...
async def test_amo_download_header(fake_session, mocker, context, tmpdir):
    ...
    with aioresponses() as m:
        ...
        with open(os.path.join(tmpdir, "test_file.bin"), "wb") as f:
            await utils.amo_download(context, "https://addons.example.com/some/api", file=f)
        assert "Authorization" in headers
        assert headers["Authorization"].startswith("JWT ")
...
@pytest.mark.parametrize("host,path", (("https://addons.example.com", "some/api"), ("https://addons.mozilla.org", "what/api/is/this/")))

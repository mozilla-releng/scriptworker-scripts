...
from snapcraft import _store as snapcraft_store_client  # TODO: Import push directly from snapcraft
...
def _release_if_needed(store, channel, snap_file_path):
    ...
    current_snap_revision, current_snap_version = _find_revision_and_version_of_current_snap(metadata_per_revision, current_sha3_384)
    latest_released_revision, latest_released_version = _pick_revision_and_version_of_latest_released_snap(channel, metadata_per_revision)

    try:
        _check_current_snap_is_not_released(current_snap_revision, current_snap_version, latest_released_revision, latest_released_version)
    ...
    release_kwargs = {"snap_name": _SNAP_NAME_ON_STORE, "revision": current_snap_revision, "channels": [channel]}
...
def _pluck_metadata(snap_revisions):
    return {
        revision["revision"]: {"version": GeckoSnapVersion.parse(revision["version"]), "current_channels": revision.get("current_channels", [])}
...
def _populate_sha3_384(store, metadata_per_revision):
    return {
        revision: {**revision_metadata, "download_sha3_384": _get_from_sha3_384_from_revision(store, revision)}
        for revision, revision_metadata in metadata_per_revision.items()
...
def _get_from_sha3_384_from_revision(store, revision):
    ...
    headers.update({"Accept": "application/hal+json", "X-Ubuntu-Series": DEFAULT_SERIES})
    params = {"fields": "status,download_sha3_384,revision", "revision": revision}
...
def _check_current_snap_is_not_released(current_revision, current_version, latest_released_revision, latest_released_version):
    if latest_released_version == current_version:
        if latest_released_revision == current_revision:
            ...
            raise TaskVerificationError(
                ...
                "Latest on store: {1}. Revision of current Snap: {2}".format(latest_released_version, latest_released_revision, current_revision)
            )
    ...
    log.debug(
        'Current version "{}" is higher than the latest released one "{}". Okay to release the current one'.format(current_version, latest_released_version)
    )
...
def _pick_revision_and_version_of_latest_released_snap(channel, metadata_per_revision):
    ...
    revision = item[0]
    version = item[1]["version"]
    log.debug('Found version "{}" (revision {}) to be the latest released on the store'.format(version, revision))
    return revision, version

...
_ALL_REVISIONS_ABSTRACT = [{
    ...
    'release_map': [{
        'series': '16', 'architecture': 'amd64', 'channel': 'beta'
    }, {
        'series': '16', 'architecture': 'amd64', 'channel': 'edge'
    }],
    'arch': 'amd64',
    'revision': 131
}, {
    ...
    'release_map': [{
        'series': '16', 'architecture': 'amd64', 'channel': 'stable'
    }, {
        'series': '16', 'architecture': 'amd64', 'channel': 'candidate'
    }],
    'arch': 'amd64',
    'revision': 124
}]
...
@pytest.mark.parametrize('channel, expected_revision', ((
    'beta', 134
), (
    'candidate', 133,
)))
def test_script_can_push_snaps_with_credentials(monkeypatch, channel, expected_revision):
    ...
    def cpi_get_side_effect(*args, **kwargs):
        ...
        cpi_get_mock.json.return_value = {
            'download_sha3_384': 'fake_hash_rev{}'.format(revision)
        }
        return cpi_get_mock
    ...
    store_mock.release.assert_called_once_with(
        snap_name='firefox', revision=expected_revision, channels=[channel]
    )
